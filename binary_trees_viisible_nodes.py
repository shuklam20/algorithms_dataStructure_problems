...
def visible_nodes(root):
  ...
  ans = {} 
  def depth_first_transverse(node, counter):
    if node is None:
      return

    depth_first_transverse(node.left, counter + 1)
    if counter not in ans:
      ans[counter] = node.val
    depth_first_transverse(node.right, counter + 1)
  
  
  depth_first_transverse(root, 0)
  return len(ans)
